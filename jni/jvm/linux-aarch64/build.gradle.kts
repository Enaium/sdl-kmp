 ...
 * Ships libsdl_jni.so as a classpath resource at
 * /cn/enaium/sdl/native/linux-aarch64/, which NativeLoader
...
val buildJniLibrary by tasks.registering(Exec::class) {
    group = "build"
    description = "Builds libsdl_jni.so for $classifier."
    onlyIf { canBuildHere }
