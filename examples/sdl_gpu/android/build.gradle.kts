...
prepareJniLibs.configure {
    androidAbis.keys.forEach { target ->
        dependsOn(project(":examples:sdl_gpu").tasks.named("linkMainDebugShared${target.replaceFirstChar { it.uppercase() }}"))
    }
}

android {
    namespace = "cn.enaium.sdl.example.gpu"
    compileSdk = 36
    defaultConfig {
        applicationId = "cn.enaium.sdl.example.gpu"
        minSdk = 24
        targetSdk = 36
        versionCode = 1
        versionName = "1.0"
