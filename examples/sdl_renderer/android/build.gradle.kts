...
prepareJniLibs.configure {
    androidAbis.keys.forEach { target ->
        dependsOn(project(":examples:sdl_renderer").tasks.named("linkMainDebugShared${target.replaceFirstChar { it.uppercase() }}"))
    }
}
...
android {
    ...
    compileSdk = 36
    defaultConfig {
        applicationId = "cn.enaium.sdl.example.renderer"
        minSdk = 24
        targetSdk = 36
        versionCode = 1
        versionName = "1.0"
