...
prepareJniLibs.configure {
    androidAbis.keys.forEach { target ->
        dependsOn(project(":examples:sdl_opengl_es").tasks.named("linkMainDebugShared${target.replaceFirstChar { it.uppercase() }}"))
    }
}

android {
    namespace = "cn.enaium.sdl.example.opengles"
    compileSdk = 36
    defaultConfig {
        applicationId = "cn.enaium.sdl.example.opengles"
        minSdk = 24
        targetSdk = 36
        versionCode = 1
        versionName = "1.0"
