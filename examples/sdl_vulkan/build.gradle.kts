...
kotlin {
    ...
    // cinterop bindings for the Vulkan demo C helper. The static library is
    ...
    targets.withType<org.jetbrains.kotlin.gradle.plugin.mpp.KotlinNativeTarget> {
        ...
        compilations.getByName("main") {
            cinterops.create("vulkan") {
                defFile(project.file("src/nativeMain/cinterop/vulkan.def"))
                includeDirs(
                ...
                extraOpts(
                    ...
                    "-staticLibrary", "libsdl_kmp_vulkan.a",
                )
            }
        }
    }
    ...
    sourceSets {
        jvm {
            mainRun {
                mainClass = "cn.enaium.sdl.example.vulkan.Main_jvmKt"
            }
        }
...
fun registerNativeBuildTasks(targetName: String, cmakeFlags: List<String> = emptyList()) {
    ...
    val cmakeBuildDir = layout.buildDirectory.dir("cmake-vulkan-$targetName").get().asFile
    ...
    val configureTask = tasks.register<Exec>("configureVulkan_$targetName") {
    ...
    val buildTask = tasks.register<Exec>("buildVulkan_$targetName") {
        onlyIf { canBuildNativeTarget(targetName) }
    ...
    tasks.matching {
        it.name.startsWith("cinteropVulkan") &&
            it.name.endsWith(targetName.replaceFirstChar { c -> c.uppercase() })
    }.configureEach {
        dependsOn(buildTask)
        inputs.file(outputDir.resolve("libsdl_kmp_vulkan.a"))
    }
}
